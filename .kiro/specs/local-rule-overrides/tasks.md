...
- [ ] 1. Storage layer and provider build pipeline
  - [ ] 1.1 Add `localRules` to storage initialization and data handling
  ...
  - [ ] 1.2 Implement `mergeProviders()` and modify provider build pipeline in `clearurls.js`
  ...
  - [ ] 1.3 Implement `reloadProviders()` in `clearurls.js`
...
- [ ] 3. Internationalization
  ...
  - [ ] 3.2 Add the same i18n message keys to all 21 non-English locale files
...
- [ ] 4. CRUD Editor page
  - [ ] 4.1 Create `html/localRules.html`
  ...
  - [ ] 4.2 Create `core_js/localRules.js` — state model and initialization
  ...
  - [ ] 4.3 Implement `renderList()`, `openAddForm()`, and `openEditForm()` in `core_js/localRules.js`
  ...
  - [ ] 4.4 Implement `markForDeletion()`, `save()`, `cancel()`, and `validateForm()` in `core_js/localRules.js`
...
- [ ] 5. Checkpoint - Verify CRUD editor page
...
- [ ] 6. Popup navigation link
  - [ ] 6.1 Add local rules link to `html/popup.html` and wire it in `core_js/popup.js`
...
- [ ] 7. Final checkpoint - Ensure all changes are integrated
